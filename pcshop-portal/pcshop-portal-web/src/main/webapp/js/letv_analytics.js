(function() {
  ...
  var oc = function() {
      ...
      ge = function(a, b, c) {
        1 <= 100 * n.random() || Aa("?") || (a = ["t=error", "_e=" + a, "_v=j37", "sr=1"], b && a[p]("_f=" + b), c && a[p]("_m=" + K(c[F](0, 100))), a[p]("aip=1"), a[p]("z=" + fe()), wc(oc(), a[H]("&"), ua))
      };
  var Ha = function() {
    this.M = []
  };
  ...
  var ab = function(a, b, c, d) {
      ...
      db = function() {};
  var gb = "letvBGTrace",//qa(f.GoogleAnalyticsObject) && sa(f.GoogleAnalyticsObject) || "ga",
      Ba = !1,
      he = S("_br"),
      hb = T("apiVersion", "v"),
        ...
        if (!Ud() && !Ba) {
          ...
          if (b && !(!O[oa][Ga] && 4 <= b && 6 >= b)) {
            var c = (new Date).getHours(),
                d = [Bd(), Bd(), Bd()][H](".");
            a = (3 == b || 5 == b ? "https:": "http:") + "//10.58.61.5:9991/logger/api/track/new?";
            a += [b, "A", c, d][H](".");
            var e = "http:";
            e = e + "//10.58.61.5:9991/logger/api/track/new?",
                e = e + [b, "B", c, d][H](".");
            7 == b && (e = e[Qc]("//www.", "//ssl."));
            c = function() {
  ...
  var Sd = function(a) {
    ...
    b && (Nd(a, 51), b = new Jd(a), Pd(b, a), Qd(b, a), a.get(b.U) && (a.set(Md, 1, !0), a.set(gd, oc(), !0)))
  };
  var Lc = function() {
      ...
      if (d.g) Yd.set(d.a[0], d.a[1]);
        ...
        if (d.i) {
          ...
          if (!ea(Yd.get(g)) && !$d.get(g)) {
            ...
            if (ca) {
              ca && 0 <= ca[t]("/") || (ca = (Ba || Ud() ? "https:": "http:") + "//10.58.61.5:9991/logger/api/track/new?" + ca);
              var l = ae(ca),
                  e = void 0;
              var k = l[E],
                  w = M[B][E],
              ...
              if (Xd = e) {
                ...
                if (e.G || 0 <= e.url[t]("?") || 0 <= e.path[t]("://")) Xd = !1;
                  ...
                  Xd = "10.58.61.5:9991/logger/api/track/new?" == e.host && (e[ja] || ce) == ce && D(e.path, "/plugins/") ? !0 : !1
                }
              }
      ...
      if (!Ud() && !Ba) {
        a: {
          for (var b = M.getElementsByTagName("script"), c = 0; c < b[y] && 100 > c; c++) {
            var d = b[c].src;
            if (d && 0 == d[t]("http://10.58.61.5:9991/logger/api/track/new?")) {
              J(33);
              b = !0;
              break a
            }
...
function letvAlysVersion() {
  var version = '1.0';
  return version;
}
function letvAlysTrackPageview() {
  var referer = letvAlysGetReferrer();
  letvBGTrace('send', 'pageview', referer);
};
function letvAlysGetCookie(name) {
    //console.log("----->>>letvAlysGetCookie("+name+") document.cookie =  " +  document.cookie);
...
var letvAlysSetCookie = function(name, value, opt) {
  opt || (opt = {});
  var t = new Date(),
      exp = opt.exp;
  ...
  document.cookie = name + "=" + escape(value) + (t && "; expires=" + t.toGMTString()) + "; domain=" + rootDomain + ";path=" + (opt.path || "/") + (opt.secure ? "; secure": "")
};

function getRootDomain(){
  var result = /(?:\.?)\w+\.(?:com|cn|org|info|\.com\.cn|co)/.exec(location.host);
...
function letvAlysSendAction(action, category, prop){
  var tj2_lc = letvAlysGenCookie(); //自己生成的letvcookie
  var uid = letvAlysGetCookie("COOKIE_USER_ID"); //用户uid的cookie
  prop = "from=pc_08&uid="+uid+"&device_id="+tj2_lc+"&"+prop;
  letvBGTrace('send', 'event', category, action, prop, 1);
}

function letvAlysGenString(){
  var lc = "";
  var i = 32;
  while (i--) {
...
function letvAlysGenCookie(){
    var letvcookie = letvAlysGetCookie("tj2_lc");
    //console.log("----->>>letvAlysGetCookie('tj2_lc') : " + letvcookie);
  if(!letvcookie){
    letvcookie = letvAlysGenString();
    letvAlysSetCookie("tj2_lc", letvcookie, {exp: "forever"});
  }
  return letvcookie;
}

function letvAlysGenSourceUrl(){
	var sourceUrl = letvAlysGetCookie("sourceUrl");
	if(!sourceUrl){
		if(document.referrer){
		  sourceUrl = document.referrer;
		  letvAlysSetCookie("sourceUrl", sourceUrl, {exp: 0.5});
		}
	}
	return sourceUrl;
}

function letvAlysGenSessionId(){
    var tj_sg = letvAlysGetCookie("tj_sg"); //标识用户是否关闭浏览器
	var tj_sid = letvAlysGetCookie("tj_sid");
	if(tj_sg==null || tj_sg==""){
	    //说明用户新打开浏览器，重新生成sid
		tj_sid = letvAlysGenString();
		letvAlysSetCookie("tj_sg", "1", {}); //有效期session 
		letvAlysSetCookie("tj_sid", tj_sid, {exp: 0.5}); //有效期1hour
	}else{
	    if(tj_sid==null||tj_sid==""){
			...
			letvAlysSetCookie("tj_sid", tj_sid, {exp: 0.5}); //更新有效期 有效期1hour
		}
	}
	return tj_sid;
}

function letvAlysInit(product, platform, country, userid_key, ext, property){
  var tj2_lc = letvAlysGenCookie(); //自己生成的letvcookie
  var sourceUrl = letvAlysGenSourceUrl(); //进来第一个页面
  var  sid = letvAlysGenSessionId(); //用户sessionid
  
  
  var tj_lc = letvAlysGetCookie("tj_lc"); //得到fashStorage生成的cookie
  var uid = letvAlysGetCookie(userid_key); //用户uid的cookie

  var cpsid = letvAlysGetCookie("COOKIE_CPS_ID");
  //var sid = letvAlysGetCookie("COOKIE_SESSION_ID"); //用户sessionid cookie 
  
  
  var clientId = 'uid=' + uid + '&letvcookie=' + tj2_lc + '&platform=' + platform
	  + '&cpsid=' + cpsid + '&sourceUrl=' + sourceUrl + '&sid=' + sid + '&eid=' + tj_lc +
     '&ext=' + ext + '&country=' + country;
  letvBGTrace('create',product, {
	'storage': 'none',
	'clientId': clientId
  });
  letvAlysTrackPageview();
}

 // old Impl
function letvAlysLoadScript(url, callback) {
...
function letvAlysLog(msg){
	if (window["console"]){
		console.log(msg);
	}
}

function letvAlyStart(product, platform, country, userid_key, ext, property){
    if((platform == 0)&&(country == 'cn')&&("http:" == document.location.protocol)) {
    	//console.log("I am flashcookie");
      letvAlysLoadScript(" http://jstatic.letvcdn.com/sdk/flashstorage_sdk.js",function(){
        //加载之后延迟 100ms 上报数据，防止swf 没执行完成
        setTimeout(function(){
          try{
           var lc = window.FlashStorageSDK.getLetvCookie();
           //兼容以前cookie 写入时没加domain
           var rootDomain = getRootDomain();
           if(rootDomain!=null && rootDomain != "" ){
              //删除到以前默认的domain的 tj_lc
              deleteCookie('tj_lc',"/",null);
           }
           //letvAlysLog("--->>>FlashStorage lc:" + lc);
          }catch(err){
           letvAlysLog("--->>>err:" + err);
           }
          //1,业务方名称 2,平台 pc:0
          letvAlysInit(product, platform, country, userid_key, ext, property);
        },100);//settimeout end
        });//loadScript end
    } else {
    	//console.log("I am cookie");
        try{
        //兼容以前cookie 写入时没加domain
        var rootDomain = getRootDomain();
        if(rootDomain!=null && rootDomain != "" ){
...
letvAlyStart("lemall", 0, "cn", "ssouid", 2, null);